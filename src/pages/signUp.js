...
const handleSubmit=async(e)=>{
   e.preventDefault();
   try{
      await axios.post('https://gptclone-ydrg.onrender.com/api/v1/auth/register',{username,email,password})
   ...
   <Collapse in={error}>
    <Alert severity='error' sx={{mb:2}}>{error}</Alert>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <TextField label="username" required margin="normal" fullWidth value={username}
          onChange={(e)=>{
        ...
        />
        <TextField label="password" required margin="normal" fullWidth
         value={password}
         type="password"
          onChange={(e)=>{
